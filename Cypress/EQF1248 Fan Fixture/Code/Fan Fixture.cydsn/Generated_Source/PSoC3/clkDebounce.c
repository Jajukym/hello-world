...
/*******************************************************************************
* Function Name: clkDebounce_Start
********************************************************************************
*
* Summary:
...
void clkDebounce_Start(void) 
{
    ...
    clkDebounce_CLKEN |= clkDebounce_CLKEN_MASK;
	clkDebounce_CLKSTBY |= clkDebounce_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: clkDebounce_Stop
********************************************************************************
*
* Summary:
...
void clkDebounce_Stop(void) 
{
    ...
    clkDebounce_CLKEN &= (uint8)(~clkDebounce_CLKEN_MASK);
	clkDebounce_CLKSTBY &= (uint8)(~clkDebounce_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: clkDebounce_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((clkDebounce_CLKEN & clkDebounce_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = clkDebounce_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clkDebounce_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* clkDebounce__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(clkDebounce_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clkDebounce_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clkDebounce_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: clkDebounce_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = clkDebounce_GetSourceRegister();
    uint16 oldDivider = clkDebounce_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clkDebounce_CLKEN & clkDebounce_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clkDebounce_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: clkDebounce_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(clkDebounce_DIV_PTR);
}


/*******************************************************************************
* Function Name: clkDebounce_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    clkDebounce_MOD_SRC |= modeBitMask & (uint8)clkDebounce_MODE_MASK;
}


/*******************************************************************************
* Function Name: clkDebounce_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    clkDebounce_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clkDebounce_MODE_MASK));
}


/*******************************************************************************
* Function Name: clkDebounce_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return clkDebounce_MOD_SRC & (uint8)(clkDebounce_MODE_MASK);
}


/*******************************************************************************
* Function Name: clkDebounce_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = clkDebounce_GetDividerRegister();
    uint8 oldSrc = clkDebounce_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clkDebounce_MOD_SRC |= CYCLK_SSS;
        clkDebounce_MOD_SRC =
            (clkDebounce_MOD_SRC & (uint8)(~clkDebounce_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clkDebounce_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return clkDebounce_MOD_SRC & clkDebounce_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clkDebounce_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    clkDebounce_PHASE = clkPhase & clkDebounce_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clkDebounce_GetPhase
********************************************************************************
*
* Summary:
...
{
    return clkDebounce_PHASE & clkDebounce_PHASE_MASK;
}
