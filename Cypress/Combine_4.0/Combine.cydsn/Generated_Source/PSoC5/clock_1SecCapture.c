...
/*******************************************************************************
* Function Name: clock_1SecCapture_Start
********************************************************************************
*
* Summary:
...
void clock_1SecCapture_Start(void) 
{
    ...
    clock_1SecCapture_CLKEN |= clock_1SecCapture_CLKEN_MASK;
	clock_1SecCapture_CLKSTBY |= clock_1SecCapture_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: clock_1SecCapture_Stop
********************************************************************************
*
* Summary:
...
void clock_1SecCapture_Stop(void) 
{
    ...
    clock_1SecCapture_CLKEN &= (uint8)(~clock_1SecCapture_CLKEN_MASK);
	clock_1SecCapture_CLKSTBY &= (uint8)(~clock_1SecCapture_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: clock_1SecCapture_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((clock_1SecCapture_CLKEN & clock_1SecCapture_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = clock_1SecCapture_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clock_1SecCapture_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* clock_1SecCapture__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(clock_1SecCapture_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clock_1SecCapture_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clock_1SecCapture_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: clock_1SecCapture_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = clock_1SecCapture_GetSourceRegister();
    uint16 oldDivider = clock_1SecCapture_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clock_1SecCapture_CLKEN & clock_1SecCapture_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(clock_1SecCapture_DIV_PTR, clkDivider);
                clock_1SecCapture_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                clock_1SecCapture_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(clock_1SecCapture_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clock_1SecCapture_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(clock_1SecCapture_DIV_PTR, clkDivider);
				clock_1SecCapture_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: clock_1SecCapture_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(clock_1SecCapture_DIV_PTR);
}


/*******************************************************************************
* Function Name: clock_1SecCapture_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    clock_1SecCapture_MOD_SRC |= modeBitMask & (uint8)clock_1SecCapture_MODE_MASK;
}


/*******************************************************************************
* Function Name: clock_1SecCapture_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    clock_1SecCapture_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clock_1SecCapture_MODE_MASK));
}


/*******************************************************************************
* Function Name: clock_1SecCapture_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return clock_1SecCapture_MOD_SRC & (uint8)(clock_1SecCapture_MODE_MASK);
}


/*******************************************************************************
* Function Name: clock_1SecCapture_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = clock_1SecCapture_GetDividerRegister();
    uint8 oldSrc = clock_1SecCapture_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clock_1SecCapture_MOD_SRC |= CYCLK_SSS;
        clock_1SecCapture_MOD_SRC =
            (clock_1SecCapture_MOD_SRC & (uint8)(~clock_1SecCapture_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clock_1SecCapture_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return clock_1SecCapture_MOD_SRC & clock_1SecCapture_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clock_1SecCapture_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    clock_1SecCapture_PHASE = clkPhase & clock_1SecCapture_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clock_1SecCapture_GetPhase
********************************************************************************
*
* Summary:
...
{
    return clock_1SecCapture_PHASE & clock_1SecCapture_PHASE_MASK;
}
