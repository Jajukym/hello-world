/*******************************************************************************
* File Name: adcVFan_Ext_CP_Clk.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: adcVFan_Ext_CP_Clk_Start
********************************************************************************
*
* Summary:
...
void adcVFan_Ext_CP_Clk_Start(void) 
{
    ...
    adcVFan_Ext_CP_Clk_CLKEN |= adcVFan_Ext_CP_Clk_CLKEN_MASK;
	adcVFan_Ext_CP_Clk_CLKSTBY |= adcVFan_Ext_CP_Clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: adcVFan_Ext_CP_Clk_Stop
********************************************************************************
*
* Summary:
...
void adcVFan_Ext_CP_Clk_Stop(void) 
{
    ...
    adcVFan_Ext_CP_Clk_CLKEN &= (uint8)(~adcVFan_Ext_CP_Clk_CLKEN_MASK);
	adcVFan_Ext_CP_Clk_CLKSTBY &= (uint8)(~adcVFan_Ext_CP_Clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: adcVFan_Ext_CP_Clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((adcVFan_Ext_CP_Clk_CLKEN & adcVFan_Ext_CP_Clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = adcVFan_Ext_CP_Clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = adcVFan_Ext_CP_Clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* adcVFan_Ext_CP_Clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(adcVFan_Ext_CP_Clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(adcVFan_Ext_CP_Clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: adcVFan_Ext_CP_Clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: adcVFan_Ext_CP_Clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = adcVFan_Ext_CP_Clk_GetSourceRegister();
    uint16 oldDivider = adcVFan_Ext_CP_Clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = adcVFan_Ext_CP_Clk_CLKEN & adcVFan_Ext_CP_Clk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(adcVFan_Ext_CP_Clk_DIV_PTR, clkDivider);
                adcVFan_Ext_CP_Clk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                adcVFan_Ext_CP_Clk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(adcVFan_Ext_CP_Clk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((adcVFan_Ext_CP_Clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(adcVFan_Ext_CP_Clk_DIV_PTR, clkDivider);
				adcVFan_Ext_CP_Clk_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: adcVFan_Ext_CP_Clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(adcVFan_Ext_CP_Clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: adcVFan_Ext_CP_Clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    adcVFan_Ext_CP_Clk_MOD_SRC |= modeBitMask & (uint8)adcVFan_Ext_CP_Clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: adcVFan_Ext_CP_Clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    adcVFan_Ext_CP_Clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(adcVFan_Ext_CP_Clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: adcVFan_Ext_CP_Clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return adcVFan_Ext_CP_Clk_MOD_SRC & (uint8)(adcVFan_Ext_CP_Clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: adcVFan_Ext_CP_Clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = adcVFan_Ext_CP_Clk_GetDividerRegister();
    uint8 oldSrc = adcVFan_Ext_CP_Clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        adcVFan_Ext_CP_Clk_MOD_SRC |= CYCLK_SSS;
        adcVFan_Ext_CP_Clk_MOD_SRC =
            (adcVFan_Ext_CP_Clk_MOD_SRC & (uint8)(~adcVFan_Ext_CP_Clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: adcVFan_Ext_CP_Clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return adcVFan_Ext_CP_Clk_MOD_SRC & adcVFan_Ext_CP_Clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: adcVFan_Ext_CP_Clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    adcVFan_Ext_CP_Clk_PHASE = clkPhase & adcVFan_Ext_CP_Clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: adcVFan_Ext_CP_Clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return adcVFan_Ext_CP_Clk_PHASE & adcVFan_Ext_CP_Clk_PHASE_MASK;
}
