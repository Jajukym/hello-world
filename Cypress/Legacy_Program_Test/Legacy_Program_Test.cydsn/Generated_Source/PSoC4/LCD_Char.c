...
void LCD_Char_WriteData(uint8 dByte) 
{
    ...
    LCD_Char_WrDatNib(nibble);
     ...
     CyDelayUs(LCD_Char_NIB_DELAY_US);
	 
    /* Write low nibble */
	nibble = dByte & LCD_Char_NIBBLE_MASK;
    LCD_Char_WrDatNib(nibble);
	
	CyDelayUs(LCD_Char_DATA_DELAY_US);
...
void LCD_Char_WriteControl(uint8 cByte) 
{
    ...
    LCD_Char_WrCntrlNib(nibble);
     ...
     CyDelayUs(LCD_Char_CMD_DELAY_US);
}
...
static void LCD_Char_WrDatNib(uint8 nibble) 
{
    ...
    LCD_Char_CNTL_REG |= LCD_Char_RS;
    /* Reset RW for write operation */
    LCD_Char_CNTL_REG &= ~LCD_Char_RW;

    /* Two following lines of code will provide us with 40ns delay */
    /* Clear data pins */
    LCD_Char_CNTL_REG &= ~LCD_Char_DATA_MASK;

    /* Write in data, bring E high*/
    LCD_Char_CNTL_REG |= (LCD_Char_E | nibble);

    /* Minimum of 230 ns delay */
    CyDelayUs(1u);

    LCD_Char_CNTL_REG &= ~LCD_Char_E;
...
static void LCD_Char_WrCntrlNib(uint8 nibble) 
{
    ...
    LCD_Char_CNTL_REG &= ~(LCD_Char_RS | LCD_Char_RW);
