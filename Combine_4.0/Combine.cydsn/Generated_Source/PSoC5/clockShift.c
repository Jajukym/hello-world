...
/*******************************************************************************
* Function Name: clockShift_Start
********************************************************************************
*
* Summary:
...
void clockShift_Start(void) 
{
    ...
    clockShift_CLKEN |= clockShift_CLKEN_MASK;
	clockShift_CLKSTBY |= clockShift_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: clockShift_Stop
********************************************************************************
*
* Summary:
...
void clockShift_Stop(void) 
{
    ...
    clockShift_CLKEN &= (uint8)(~clockShift_CLKEN_MASK);
	clockShift_CLKSTBY &= (uint8)(~clockShift_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: clockShift_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((clockShift_CLKEN & clockShift_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = clockShift_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clockShift_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* clockShift__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(clockShift_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clockShift_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clockShift_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: clockShift_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = clockShift_GetSourceRegister();
    uint16 oldDivider = clockShift_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clockShift_CLKEN & clockShift_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clockShift_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: clockShift_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(clockShift_DIV_PTR);
}


/*******************************************************************************
* Function Name: clockShift_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    clockShift_MOD_SRC |= modeBitMask & (uint8)clockShift_MODE_MASK;
}


/*******************************************************************************
* Function Name: clockShift_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    clockShift_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clockShift_MODE_MASK));
}


/*******************************************************************************
* Function Name: clockShift_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return clockShift_MOD_SRC & (uint8)(clockShift_MODE_MASK);
}


/*******************************************************************************
* Function Name: clockShift_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = clockShift_GetDividerRegister();
    uint8 oldSrc = clockShift_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clockShift_MOD_SRC |= CYCLK_SSS;
        clockShift_MOD_SRC =
            (clockShift_MOD_SRC & (uint8)(~clockShift_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clockShift_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return clockShift_MOD_SRC & clockShift_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clockShift_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    clockShift_PHASE = clkPhase & clockShift_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clockShift_GetPhase
********************************************************************************
*
* Summary:
...
{
    return clockShift_PHASE & clockShift_PHASE_MASK;
}
