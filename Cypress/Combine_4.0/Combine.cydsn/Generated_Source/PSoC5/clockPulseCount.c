...
/*******************************************************************************
* Function Name: clockPulseCount_Start
********************************************************************************
*
* Summary:
...
void clockPulseCount_Start(void) 
{
    ...
    clockPulseCount_CLKEN |= clockPulseCount_CLKEN_MASK;
	clockPulseCount_CLKSTBY |= clockPulseCount_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: clockPulseCount_Stop
********************************************************************************
*
* Summary:
...
void clockPulseCount_Stop(void) 
{
    ...
    clockPulseCount_CLKEN &= (uint8)(~clockPulseCount_CLKEN_MASK);
	clockPulseCount_CLKSTBY &= (uint8)(~clockPulseCount_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: clockPulseCount_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((clockPulseCount_CLKEN & clockPulseCount_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = clockPulseCount_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clockPulseCount_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* clockPulseCount__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(clockPulseCount_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clockPulseCount_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clockPulseCount_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: clockPulseCount_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = clockPulseCount_GetSourceRegister();
    uint16 oldDivider = clockPulseCount_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clockPulseCount_CLKEN & clockPulseCount_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(clockPulseCount_DIV_PTR, clkDivider);
                clockPulseCount_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                clockPulseCount_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(clockPulseCount_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clockPulseCount_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: clockPulseCount_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(clockPulseCount_DIV_PTR);
}


/*******************************************************************************
* Function Name: clockPulseCount_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    clockPulseCount_MOD_SRC |= modeBitMask & (uint8)clockPulseCount_MODE_MASK;
}


/*******************************************************************************
* Function Name: clockPulseCount_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    clockPulseCount_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clockPulseCount_MODE_MASK));
}


/*******************************************************************************
* Function Name: clockPulseCount_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return clockPulseCount_MOD_SRC & (uint8)(clockPulseCount_MODE_MASK);
}


/*******************************************************************************
* Function Name: clockPulseCount_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = clockPulseCount_GetDividerRegister();
    uint8 oldSrc = clockPulseCount_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clockPulseCount_MOD_SRC |= CYCLK_SSS;
        clockPulseCount_MOD_SRC =
            (clockPulseCount_MOD_SRC & (uint8)(~clockPulseCount_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clockPulseCount_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return clockPulseCount_MOD_SRC & clockPulseCount_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clockPulseCount_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    clockPulseCount_PHASE = clkPhase & clockPulseCount_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clockPulseCount_GetPhase
********************************************************************************
*
* Summary:
...
{
    return clockPulseCount_PHASE & clockPulseCount_PHASE_MASK;
}
