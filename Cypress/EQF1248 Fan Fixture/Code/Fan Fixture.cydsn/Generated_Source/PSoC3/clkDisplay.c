...
/*******************************************************************************
* Function Name: clkDisplay_Start
********************************************************************************
*
* Summary:
...
void clkDisplay_Start(void) 
{
    ...
    clkDisplay_CLKEN |= clkDisplay_CLKEN_MASK;
	clkDisplay_CLKSTBY |= clkDisplay_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: clkDisplay_Stop
********************************************************************************
*
* Summary:
...
void clkDisplay_Stop(void) 
{
    ...
    clkDisplay_CLKEN &= (uint8)(~clkDisplay_CLKEN_MASK);
	clkDisplay_CLKSTBY &= (uint8)(~clkDisplay_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: clkDisplay_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((clkDisplay_CLKEN & clkDisplay_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = clkDisplay_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clkDisplay_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* clkDisplay__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(clkDisplay_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clkDisplay_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clkDisplay_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: clkDisplay_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = clkDisplay_GetSourceRegister();
    uint16 oldDivider = clkDisplay_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clkDisplay_CLKEN & clkDisplay_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clkDisplay_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: clkDisplay_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(clkDisplay_DIV_PTR);
}


/*******************************************************************************
* Function Name: clkDisplay_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    clkDisplay_MOD_SRC |= modeBitMask & (uint8)clkDisplay_MODE_MASK;
}


/*******************************************************************************
* Function Name: clkDisplay_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    clkDisplay_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clkDisplay_MODE_MASK));
}


/*******************************************************************************
* Function Name: clkDisplay_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return clkDisplay_MOD_SRC & (uint8)(clkDisplay_MODE_MASK);
}


/*******************************************************************************
* Function Name: clkDisplay_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = clkDisplay_GetDividerRegister();
    uint8 oldSrc = clkDisplay_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clkDisplay_MOD_SRC |= CYCLK_SSS;
        clkDisplay_MOD_SRC =
            (clkDisplay_MOD_SRC & (uint8)(~clkDisplay_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clkDisplay_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return clkDisplay_MOD_SRC & clkDisplay_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clkDisplay_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    clkDisplay_PHASE = clkPhase & clkDisplay_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clkDisplay_GetPhase
********************************************************************************
*
* Summary:
...
{
    return clkDisplay_PHASE & clkDisplay_PHASE_MASK;
}
