...
/*******************************************************************************
* Function Name: clockButtonRead_Start
********************************************************************************
*
* Summary:
...
void clockButtonRead_Start(void) 
{
    ...
    clockButtonRead_CLKEN |= clockButtonRead_CLKEN_MASK;
	clockButtonRead_CLKSTBY |= clockButtonRead_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: clockButtonRead_Stop
********************************************************************************
*
* Summary:
...
void clockButtonRead_Stop(void) 
{
    ...
    clockButtonRead_CLKEN &= (uint8)(~clockButtonRead_CLKEN_MASK);
	clockButtonRead_CLKSTBY &= (uint8)(~clockButtonRead_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: clockButtonRead_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((clockButtonRead_CLKEN & clockButtonRead_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = clockButtonRead_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clockButtonRead_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* clockButtonRead__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(clockButtonRead_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clockButtonRead_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clockButtonRead_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: clockButtonRead_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = clockButtonRead_GetSourceRegister();
    uint16 oldDivider = clockButtonRead_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clockButtonRead_CLKEN & clockButtonRead_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(clockButtonRead_DIV_PTR, clkDivider);
                clockButtonRead_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                clockButtonRead_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(clockButtonRead_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clockButtonRead_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: clockButtonRead_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(clockButtonRead_DIV_PTR);
}


/*******************************************************************************
* Function Name: clockButtonRead_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    clockButtonRead_MOD_SRC |= modeBitMask & (uint8)clockButtonRead_MODE_MASK;
}


/*******************************************************************************
* Function Name: clockButtonRead_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    clockButtonRead_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clockButtonRead_MODE_MASK));
}


/*******************************************************************************
* Function Name: clockButtonRead_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return clockButtonRead_MOD_SRC & (uint8)(clockButtonRead_MODE_MASK);
}


/*******************************************************************************
* Function Name: clockButtonRead_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = clockButtonRead_GetDividerRegister();
    uint8 oldSrc = clockButtonRead_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clockButtonRead_MOD_SRC |= CYCLK_SSS;
        clockButtonRead_MOD_SRC =
            (clockButtonRead_MOD_SRC & (uint8)(~clockButtonRead_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clockButtonRead_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return clockButtonRead_MOD_SRC & clockButtonRead_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clockButtonRead_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    clockButtonRead_PHASE = clkPhase & clockButtonRead_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clockButtonRead_GetPhase
********************************************************************************
*
* Summary:
...
{
    return clockButtonRead_PHASE & clockButtonRead_PHASE_MASK;
}
