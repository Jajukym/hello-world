...
/*******************************************************************************
* Function Name: eepromTime_Enable
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: eepromTime_Start
********************************************************************************
*
* Summary:
*  Starts EEPROM.
...
/*******************************************************************************
* Function Name: eepromTime_WriteByte
********************************************************************************
*
* Summary:
...
*  call eepromTime_UpdateTemperature() function if the temperature of the
    ...
    if (address < CY_EEPROM_SIZE)
    {
        ...
        if(CYRET_SUCCESS == CySpcLock())
        {
            status = CySpcLoadMultiByte(CY_SPC_FIRST_EE_ARRAYID, byteNumber, &dataByte, \
                                                                    eepromTime_SPC_BYTE_WRITE_SIZE);
...
/*******************************************************************************
* Function Name: eepromTime_ReadByte
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: eepromTime_UpdateTemperature
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: eepromTime_EraseSector
********************************************************************************
*
* Summary:
...
cystatus eepromTime_EraseSector(uint8 sectorNumber) 
{
    ...
    if(sectorNumber < (uint8) eepromTime_SECTORS_NUMBER)
    {
...
/*******************************************************************************
* Function Name: eepromTime_Write
********************************************************************************
*
* Summary:
...
*  eepromTime_UpdateTemperature() function if the temperature of the
...
/*******************************************************************************
* Function Name: eepromTime_StartWrite
********************************************************************************
*
* Summary:
...
*  eepromTime_Query(). eepromTime_Query() must be called
*  until it returns a status other than CYRET_STARTED. That indicates that the
*  write has completed. Until eepromTime_Query() detects that
...
*  you should call eepromTime_UpdateTemperature() API if the temperature
...
/*******************************************************************************
* Function Name: eepromTime_StartErase
********************************************************************************
*
* Summary:
...
*  must be used in combination with eepromTime_Query().
*  eepromTime_Query() must be called until it returns a status
*  other than CYRET_STARTED. That indicates the erase has been completed.
*  Until eepromTime_Query() detects that the erase is
...
/*******************************************************************************
* Function Name: eepromTime_Query
********************************************************************************
*
* Summary:
*  Checks the status of an earlier call to eepromTime_StartWrite() or
*  eepromTime_StartErase().
...
/*******************************************************************************
* Function Name: eepromTime_ByteWritePos
********************************************************************************
*
* Summary:
    ...
    if((rowNumber < (uint8) CY_EEPROM_NUMBER_ROWS) && (byteNumber < (uint8) SIZEOF_EEPROM_ROW))
    {
        ...
        if(CySpcLock() == CYRET_SUCCESS)
        {
            ...
            if(CySpcLoadMultiByte(CY_SPC_FIRST_EE_ARRAYID, (uint16)byteNumber, &dataByte,\
                                                                eepromTime_SPC_BYTE_WRITE_SIZE) == CYRET_STARTED)
            {
                while(CY_SPC_BUSY)
                {
                    /* Wait until SPC becomes idle */
