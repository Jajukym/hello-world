...
void clkPWM2_Start(void) 
{
    ...
    clkPWM2_CLKEN |= clkPWM2_CLKEN_MASK;
	clkPWM2_CLKSTBY |= clkPWM2_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: clkPWM2_Stop
********************************************************************************
*
* Summary:
...
void clkPWM2_Stop(void) 
{
    ...
    clkPWM2_CLKEN &= (uint8)(~clkPWM2_CLKEN_MASK);
	clkPWM2_CLKSTBY &= (uint8)(~clkPWM2_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: clkPWM2_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((clkPWM2_CLKEN & clkPWM2_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = clkPWM2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clkPWM2_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(clkPWM2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clkPWM2_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clkPWM2_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: clkPWM2_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = clkPWM2_GetSourceRegister();
    uint16 oldDivider = clkPWM2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clkPWM2_CLKEN & clkPWM2_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clkPWM2_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: clkPWM2_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(clkPWM2_DIV_PTR);
}


/*******************************************************************************
* Function Name: clkPWM2_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    clkPWM2_MOD_SRC |= modeBitMask & (uint8)clkPWM2_MODE_MASK;
}


/*******************************************************************************
* Function Name: clkPWM2_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    clkPWM2_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clkPWM2_MODE_MASK));
}


/*******************************************************************************
* Function Name: clkPWM2_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return clkPWM2_MOD_SRC & (uint8)(clkPWM2_MODE_MASK);
}


/*******************************************************************************
* Function Name: clkPWM2_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = clkPWM2_GetDividerRegister();
    uint8 oldSrc = clkPWM2_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clkPWM2_MOD_SRC |= CYCLK_SSS;
        clkPWM2_MOD_SRC =
            (clkPWM2_MOD_SRC & (uint8)(~clkPWM2_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clkPWM2_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return clkPWM2_MOD_SRC & clkPWM2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clkPWM2_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    clkPWM2_PHASE = clkPhase & clkPWM2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clkPWM2_GetPhase
********************************************************************************
*
* Summary:
...
{
    return clkPWM2_PHASE & clkPWM2_PHASE_MASK;
}
